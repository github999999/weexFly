package com.example.weexdemo.base;
...
import com.example.weexdemo.utils.WeexConstants;
import com.example.weexdemo.utils.WeexFileUtil;
...
//            wxsdkInstance.renderByUrl(getPageName(),
//                    path,options,null,WXRenderStrategy.APPEND_ONCE);
//        }
    }
...
//    }
   ...
   // 可参考的方法
    protected void render(String mUri, HashMap mConfig) {
        if (mUri == null) return;
        if (!mUri.startsWith("http")) {
            String template = null;
            if (mUri.startsWith("file:///android_asset/")){
                mUri = mUri.replace("file:///android_asset/", "");
                template = WXFileUtils.loadAsset(mUri, this);
            } else if(mUri.startsWith("file://assets/")) {
                mUri = mUri.replace("file://assets/", "");
                template = WXFileUtils.loadAsset(mUri, this);
            } else {
                template = WXFileUtils.loadFileOrAsset(mUri, this);
            }
            wxsdkInstance.render(getPageName(), template,
                    mConfig,
                    null,
                    WXRenderStrategy.APPEND_ASYNC);
        } else if (mUri.startsWith("http")) {
            wxsdkInstance.renderByUrl(getPageName(), mUri,
                    mConfig,
                    null,
                    WXRenderStrategy.APPEND_ASYNC);
        }
